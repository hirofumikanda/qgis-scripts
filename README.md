...
このプロジェクトは、QGIS（Quantum GIS）環境で直接実行可能なPythonスクリプトのコレクションです。各スクリプトはQGISのPythonコンソールで実行され、`iface`オブジェクトを通じてマップキャンバスとのリアルタイムインタラクションを行います。
...
地図のパンやズームに応じて自動的にWebメルカトルタイル境界を更新表示するメモリレイヤーを作成します。

**機能:**
- 地図移動・拡大縮小時のリアルタイム自動更新
- 現在の地図表示範囲に対応するタイル境界の表示
- EPSG:3857（Webメルカトル）座標系でのタイル計算
- ズームレベルは自動推定で設定
- 透明な塗りつぶしと赤い境界線でのスタイリング
- タイル座標ラベル表示（z/x/y形式）
...
3. スクリプトファイルを読み込んで実行
4. 地図をパンまたはズームすると自動的にタイル境界が更新される
